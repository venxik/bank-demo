...
**Q: Cache improves performance but starts serving stale data — how would you fix it?**
A: Means the cache-invalidation half of the strategy is missing or too loose — a cache with no clear invalidation story reliably ages into "fast but wrong." Match the fix to why it's stale: if the TTL is simply too long for how often the underlying data actually changes, shorten it (direct tradeoff — shorter TTL means fresher data but more DB load). If writes don't proactively touch the cache at all, add that — a write-through update or an explicit evict on every write path that touches that row, so a write is never followed by a stale read from someone else. If multiple instances each hold their own local (in-process) cache, a write on one instance never invalidates the copy sitting in every other instance's memory — either move to a shared cache (Redis) every instance reads from, or broadcast an invalidation event (a Kafka/pub-sub message every instance subscribes to) when something changes.

```java
// write-through invalidation — every write immediately updates/evicts the cache, not just the DB
@Transactional
public AccountResponse updateBalance(Long id, BigDecimal amount) {
    Account account = accountRepository.save(...);
    cache.evict(id); // or cache.put(id, account) — either way, the next read can't see stale data
...
**Q: API works locally but becomes slow in production — how do you debug it?**
A: "Works locally" environments almost never match production's actual conditions, and that gap is usually the whole answer. Compare, specifically: **data volume** (local dev DBs are usually tiny — an unindexed query is fine on 100 rows, terrible on 10 million); **real network latency** (a downstream call is instant on localhost, but genuinely takes time across a VPC/region in prod); **connection pool sizing** against real concurrent load rather than one local user; and **prod-only config** (a stricter timeout, a different fetch/`ddl-auto` setting, an extra proxy hop). This is the same "isolate where the time is going" methodology above, with one extra first step: diff the environment before assuming the code changed, since it usually didn't.

**Q: Application is healthy, but users are still getting timeouts — what would you investigate?**
A: "Healthy" almost always means a shallow check passed — `/actuator/health` returning `UP` only confirms the process is running and its immediate dependencies respond, and says nothing about whether real user requests complete in time. Investigate: is the health check itself representative of the actual request path (a lightweight ping vs. the heavier endpoints users actually hit)? Is the timeout happening *before* the request even reaches your service — a load balancer, API gateway, or client-side timeout set more aggressively than your real p99? Is one specific slow endpoint dragging down the picture while most requests are fine (check per-endpoint latency, not an overall average)? And is a downstream dependency slow without being fully down — health checks typically test reachability, not latency, so a dependency responding in 8 seconds instead of 200ms still reports healthy while quietly timing out every request that goes through it.

**Q: API response is slow even though the database query is fast — where else could the bottleneck be?**
A: Directly the "isolate where the time is going" step above, applied to the one case where the obvious suspect is already ruled out. Once the query itself is confirmed fast, look at: serialization overhead on a large response payload (mapping a big result set to DTOs, then to JSON, is real CPU work); thread pool exhaustion — the query runs fast, but the request sat waiting for a free worker thread before it even started; N+1 queries *elsewhere* in the same request (the slow part isn't one slow query, it's 200 other individually-fast queries adding up); GC pauses stealing wall-clock time between otherwise-fast steps; or a downstream call — an auth check, another service — that has nothing to do with the database at all. Confirm with a trace rather than assuming "database," since that's usually the reflex answer and it's already been ruled out here.

**Q: Application works perfectly in staging but fails in production — what differences would you compare?**
A: A checklist for exactly the gap staging is supposed to close but often doesn't: **configuration** (env vars, feature flags, secrets that differ or are missing in prod); **data** (staging data is usually smaller and cleaner than real production data — an edge case that only exists in messy real-world rows); **scale/concurrency** (staging rarely sees real concurrent load, so a race condition or pool-sizing issue only surfaces under production traffic); **infrastructure** (network topology, firewall/security-group rules, a proxy or load balancer in prod that staging doesn't have); and **external dependencies** (staging often talks to a sandboxed or mocked third-party API that behaves differently — different rate limits, different response shapes — than the real one). Work through these five methodically rather than guessing; "works in staging" bugs are almost always one of these, not a code difference between the two environments.

**Q: A service crashes only under heavy load — how would you identify the actual root cause?**
A: "Only under load" narrows it to resource exhaustion or a concurrency bug — something fine at low volume that breaks past some threshold. Reproduce deliberately with a load test (the load-vs-stress-testing distinction in Part 15) rather than waiting for it to happen in production again — this turns an unreproducible incident into a controlled, repeatable one you can actually attach a profiler or debugger to. Once reproducible, check, roughly in order: thread pool exhaustion (every worker blocked waiting on something, new requests queue and eventually time out); connection pool exhaustion (Part 4); memory pressure (an allocation pattern that's fine at low volume but runs out of heap once enough concurrent requests are in flight); a deadlock or race condition that only statistically manifests once enough threads interleave at the same moment (rare at low concurrency, near-certain at high concurrency); or a downstream dependency with its own capacity limit that starts failing once your load exceeds it. A thread dump (`jstack`) taken *during* a reproduced crash under load is usually the single most informative artifact — it shows exactly what every thread was doing, and blocked on, at the moment of failure.
